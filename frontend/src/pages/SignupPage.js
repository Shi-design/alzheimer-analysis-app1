      ...
      <div className="w-full max-w-6xl bg-white/90 rounded-2xl shadow-2xl overflow-hidden grid md:grid-cols-2">
        ...
        <div className="p-8 sm:p-12 flex flex-col justify-center">
          ...
          <form onSubmit={onSubmit} className="space-y-6 max-w-sm mx-auto">
            <input name="name" value={form.name} onChange={onChange} placeholder="Full Name" required className="block w-full px-4 py-3 border border-gray-300 rounded-lg" />
            <input name="email" type="email" value={form.email} onChange={onChange} placeholder="Email" required className="block w-full px-4 py-3 border border-gray-300 rounded-lg" />
          ...
          <p className="text-center text-gray-500 mt-6">
            Already have an account? <Link to="/" className="text-indigo-600 font-semibold">Sign In</Link>
          </p>
        </div>
      </div>
    </div>
